// CAlias.cpp: 구현 파일
//

#include "stdafx.h"
#include "SocketClient.h"
#include "SocketClientDlg.h"
#include "CAlias.h"
#include "afxdialogex.h"


// CAlias 대화 상자

IMPLEMENT_DYNAMIC(CAlias, CDialogEx)

CAlias::CAlias(CWnd* pParent /*=nullptr*/)
	: CDialogEx(IDD_DIALOG1, pParent)
{

}

CAlias::~CAlias()
{
}

void CAlias::DoDataExchange(CDataExchange* pDX)
{

	CButton *button1 = (CButton*)GetDlgItem(IDOK);
	CButton *button2 = (CButton*)GetDlgItem(IDCANCEL);
	button1->DestroyWindow();
	button2->DestroyWindow();

	CDialogEx::DoDataExchange(pDX);
}

BEGIN_MESSAGE_MAP(CAlias, CDialogEx)
	ON_BN_CLICKED(IDC_ALIAS, &CAlias::OnBnClickedAlias)
END_MESSAGE_MAP()


// CAlias 메시지 처리기


void CAlias::OnBnClickedAlias()
{
	CString str;
	CString str1;
	CString str2;
	CString USERNAME;
	CString alias = L"alias:";

	GetDlgItemText(IDC_EDIT1, str);
	((CSocketClientDlg *)GetParent())->name = str;

	str1.Format(L"SELECT win, lose FROM yut_table WHERE name = '%s'", str);

	USERNAME.Format(_T("%s님 환영합니다."), str);

	try {
		CString Data[7] = { L"" };
		BOOL bOpen = m_prs->Open(CRecordset::snapshot, str1);
		if (bOpen)
		{
			CSocketClientDlg* pMain = (CSocketClientDlg*)AfxGetMainWnd();
			int Row = 1;
			BOOL bIsEOF = m_prs->IsEOF();
			if (bIsEOF) {
				str2.Format(_T("INSERT INTO yut_table(name, win, lose) VALUES('%s','%d','%d')"), str, 0, 0);
				m_db.ExecuteSQL(str2);

				pMain->Win.Format(_T("0"));
				pMain->Lose.Format(_T("0"));

			}
			if (!bIsEOF)
			{
				AfxMessageBox(USERNAME);

				CSocketClientDlg* pMain = (CSocketClientDlg*)AfxGetMainWnd();
				for (m_prs->MoveFirst(); !m_prs->IsEOF(); m_prs->MoveNext()) {
					int iFieldCnt = m_prs->GetODBCFieldCount();
					for (int Col = 0; Col < iFieldCnt; Col++) {
						CString sItem;
						m_prs->SetAbsolutePosition(Row);
						m_prs->GetFieldValue(Col, sItem);
						Data[Col] = sItem;
					}
					pMain->Win = Data[0];
					pMain->Lose = Data[1];
				}
			}
		}
	}
	catch (CException* e) {
		e->ReportError();
		return;
	}

	str = alias + str;
	((CSocketClientDlg*)GetParent())->m_Socket.Send((LPVOID)(LPCTSTR)str, str.GetLength() * 2);

	::SendMessage(this->m_hWnd, WM_CLOSE, NULL, NULL);
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
}


BOOL CAlias::PreTranslateMessage(MSG* pMsg)
{
	// TODO: 여기에 특수화된 코드를 추가 및/또는 기본 클래스를 호출합니다.
	if (pMsg->message == WM_KEYDOWN)
	{
		if (pMsg->wParam == VK_RETURN || pMsg->wParam == VK_ESCAPE)
		{   // 위 VK_RETURN은 Enter, VK_ESCAPE는 ESC을 의미함. 필요시 하나만 사용.
			OnBnClickedAlias();
			return true;
		}
	}
	return CDialogEx::PreTranslateMessage(pMsg);
}




BOOL CAlias::OnInitDialog()
{
	CDialogEx::OnInitDialog();

	// TODO:  여기에 추가 초기화 작업을 추가합니다.
	BOOL bopen = m_db.OpenEx(_T("DSN=mydb;SERVER=155.230.235.248;PORT=3405;UID=cduser01;PWD=user01@swdb;DATABASE=swdb;"));
	if (bopen)
		m_prs = new CRecordset(&m_db);
	return TRUE;  // return TRUE unless you set the focus to a control
				  // 예외: OCX 속성 페이지는 FALSE를 반환해야 합니다.
}
